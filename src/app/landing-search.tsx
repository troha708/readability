...
export function LandingSearch({ className }: { className?: string } = {}) {
  ...
  return (
    // The search reads as a field, no rules around it: a whisper of the
    // surface tone on the dark bar, the light surface step on a light page.
    // In the header it takes its own full-width row on phones; `className`
    // lets a caller place it somewhere else, as the library's sticky bar
    // does.
    <div
      className={`relative ${
        className ??
